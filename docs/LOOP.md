...
- [x] Web 配置写入改为 RCU 更新 + DeviceActor 异步 NVS commit；HTTP 线程不再同步
  执行 Flash 操作，失败由 dirty 标志保留并自动重试，避免高并发时通信服务被保存拖死。
- [x] 系统信息、RS485 端口和 BLE 名称保存不再误触发 W5500 重配；三路 RS485
  连续 POST 不会在端口 1 保存后中断端口 2/3。
- [x] 网络配置只更新持久化快照，不在线热重配活动 netif；新 IP 在下次启动时生效，
  避免配置请求主动拆除当前 Web/TCP 连接。
...
## LOOP31 Web UI 信息架构与保存语义修正 (2026-08-14)

### 修复

- 6 个平级 Tab 重组为运行监控、基本配置、通信设置、系统维护 4 个工作区，所有原有控件和接口保持可达。
- 固件版本、MAC、DHCP、运行状态和 NFC 状态改为只读值组件，与可编辑输入框形成明确视觉区分。
- 网络参数和设备基本信息使用独立保存动作；后端支持标识字段部分更新，同时兼容旧 Web、PC 和手持机的完整参数提交。
- 网络及 RS485 配置成功提示明确说明重启后生效，保存失败提示区分具体业务区域。
- 网络及 RS485 区域增加“保存并重启”操作；配置由 DeviceActor 成功写入 NVS 后才请求复位，持久化失败时取消复位。

### 验证

- 默认、F3、F4 `cargo check` 通过，0 warning；测试目标编译通过。
- JavaScript 语法、HTML 重复 ID、390px 无横向溢出检查通过。
...
## LOOP35 Android 手持机网络配置读写修复 (2026-09-23)

- 修正 IP/掩码/网关 holding register 地址，按 Android `DeviceFragment` 实际
  FC16 线序 `IP → 掩码 → 网关` 写入；读取同样按 `IP → 掩码 → 网关` 返回。
- 手持机网络写入成功后同步持久化，NVS 成功才请求复位；普通 Modbus TCP/RTU
  保持标准寄存器顺序。
...
- 修复 Android 手持机网络写入的 Java 形参误导：实际线序为
  `IP → 掩码 → 网关`，固件不再二次交换；启动时自动迁移此前已经写反的 NVS 快照。
- 修复 `AT+CFG485` 波特率解析：原代码把输入乘以 100 且无法接受 115200，现按完整
  `u32` 波特率解析并限制在 300..921600；`CFGAPPLY` 返回文本同步为实际的无重启运行时应用。
- BLE 逻辑配置读取改用固定容量 `heapless::Vec`，避免后台路径动态分配，并拒绝超过
  256 字节响应容量的静默截断。
- RS485 mode=3 过去只记录收到的数据而不转发，现明确按 Slave 安全运行，删除“已支持
  透传”的误导性路径和 TODO。
- 更新 README、用户手册、TODO 和网络配置记录；旧审计报告保留为历史归档。
- 默认、F3、F4 特性 `cargo check`，默认构建和测试目标编译均通过，当前检查无警告。
- 真实设备启动复核：DIO/40MHz/8MB、NVS、W5500、BLE、Web、NFC、四个 Modbus TCP
  端口均正常；网络配置读回为 `192.168.51.220 / 255.255.255.0 / 192.168.51.1`。

### 仍需在正式量产前完成

- Android 手持机 BLE 端到端回归和断电复读。
- 默认固件完整协议栈 72 小时浸泡测试。
- 若产品启用 Wi-Fi 或 RS485 透传，必须先完成对应功能设计和实机验证；当前固件不宣称
  这两项能力已启用。
