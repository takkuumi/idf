...
pub fn start(hal: Arc<Hal>, sys_loop: EspSystemEventLoop) -> AppResult<()> {
    ...
    log::info!("[PROBE-eth] before eth_reset_pulse");
    ...
    log::info!("[PROBE-eth] before esp_eth_mac_new_w5500");
    let mac = unsafe { esp_idf_sys::esp_eth_mac_new_w5500(&w5500_cfg, &mac_cfg) };
    log::info!("[PROBE-eth] after esp_eth_mac_new_w5500");
    if mac.is_null() {
    ...
    log::info!("[PROBE-eth] before esp_eth_phy_new_w5500");
    let phy = unsafe { esp_idf_sys::esp_eth_phy_new_w5500(&phy_cfg) };
    log::info!("[PROBE-eth] after esp_eth_phy_new_w5500");
    if phy.is_null() {
