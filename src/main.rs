...
fn main() -> AppResult<()> {
    ...
    log::info!("[PROBE] before load_reset_count");
    let mut reset_count = device::load_reset_count();
    log::info!("[PROBE] after load_reset_count = {}", reset_count);
    ...
    log::info!("[PROBE] after set_reset_count/reason");

    // 5. 启动以太网 (W5500)
    #[cfg(feature = "ethernet-w5500")]
    {
        log::info!("[main] starting ethernet (W5500)...");
        log::info!("[PROBE] before ethernet::start");
       
        ethernet::start(hal.clone(), sys_loop.clone())?;
        log::info!("[PROBE] after ethernet::start");
    }
