...
pub const MODBUS_TCP: usize = 16 * KIB;
...
/// 默认功能集常驻用户任务栈。
pub const DEFAULT_USER_STACK_TOTAL: usize = MAIN
    + DEVICE_ACTOR
    + MODBUS_TCP
    + MODBUS_RTU_MASTER
    + MODBUS_RTU_SLAVE
...
pub const KNOWN_SYSTEM_STACK_TOTAL: usize = BLUEDROID_BTC
    + BLUEDROID_BTU
    + LWIP_TCPIP
    + SYSTEM_EVENT
    + FREERTOS_TIMER
...
const _: () = assert!(MODBUS_TCP <= 16 * KIB);
...
mod tests {
    ...
    fn test_all_user_stacks_stay_within_internal_sram_budget() {
        assert_eq!(DEFAULT_USER_STACK_TOTAL, 106 * KIB);
        assert_eq!(ALL_USER_STACK_TOTAL, 120 * KIB);
